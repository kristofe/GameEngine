...
#pragma warning( disable : 4127 )
	...
	#define _aligned_realloc(a, b, c) (realloc(a, b))
	#define _aligned_free(A) (free(A))
#endif

#define foreachindex(I, A) for (int (I) = 0; (I) < (A).size; (I)++)
#define foreach(T, V, A) for ((T) *(V) = &((A)[0]), int I = 0; I < (A).size; (V)=&((A)[I]), I++);

template <class T, bool sse_align = false, int initialCapacity = 16> class GWArray
{
	public:
	...
	~GWArray()
	{
		...
		#endif
		if(sse_align) _aligned_free(a);
		else free(a);
	}

	/*GWArray(int initialCapacity)
	{
		size = 0;
		capacity = initialCapacity;
	...
	*/
	...
	__forceinline void swapData(GWArray<T, sse_align, initialCapacity>& other)
	{
		T* tempa = a;
		a = other.a;
		other.a = tempa;
	...
	__forceinline void insert(int i, const T& e)
	{
		#ifdef ASSERT_SMART_ARRAY
			assert(i >= 0 && i < size);
		#endif
	...
	__forceinline void insertAutoSize(int i, const T& e)
	{
		#ifdef ASSERT_SMART_ARRAY
			assert(i >= 0 && i < size+1);
		#endif
